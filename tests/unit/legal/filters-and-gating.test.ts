...
describe('legalActsSpec — §7.1 vocabulary separation + aliases', () => {
  it('act_type is on acts (alias a); category/domain are on summaries (alias s)', () => {
    ...
    expect(byName.get('category')?.column).toMatchObject({ alias: 's', column: 'document_category' });
    expect(byName.get('domain')?.column).toMatchObject({ alias: 's', column: 'domains', arrayColumn: true });
  ...
  it('default sort is in_degree (R1)', () => {
    ...
    expect(legalActsSpec.sort.allowed).toEqual(['in_degree', 'act_year', 'entry_into_force', 'display_citation']);
  });
