 ...
 *   M6  financials.lines money values are strings (Money-as-string contract)
...
import { makeCompanyFinancials, makeCompanyResolve, toCompanyResolveHits, type CompanyUsecaseDeps } from '@/modules/companies/core/usecases.js';
import { mapFinancialYear, mapTerritory, type FinancialRow } from '@/modules/companies/shell/repo/mappers.js';
...
const yearRow = (year: number, netProfit: string, netLoss: string): CompanyFinancialYear => ({
  ...
  employees: null,
  summary: {
    turnover: '0.00', netProfit, netLoss, totalRevenue: null, totalExpenses: null, grossProfit: null,
...
describe('H1 — netResultDelta nets profit against loss', () => {
  it('uses (profit - loss), not profit-only, across a loss→profit swing', async () => {
    ...
    const getFinancials = vi.fn(async () => ok([yearRow(2024, '5931214.00', '0.00'), yearRow(2023, '0.00', '4683875.00')]));
  ...
  it('is null only when both profit and loss are absent', async () => {
    const both = (y: number): CompanyFinancialYear => ({ ...yearRow(y, '0.00', '0.00'), netProfit: null, netLoss: null });
...
describe('M4 — territory emits an explicit unmatched object', () => {
  it('returns matchConfidence=unmatched when a registration exists but SIRUTA missed', () => {
    const t = mapTerritory({ uat_siruta_code: null, uat_name: null, county_name: null, match_confidence: 'unmatched' });
  ...
  it('returns null only when there is no registration data at all', () => {
    const t = mapTerritory({ uat_siruta_code: null, uat_name: null, county_name: null, match_confidence: null });
    expect(t).toBeNull();
  });

  it('returns safe with codes when SIRUTA matched', () => {
    const t = mapTerritory({ uat_siruta_code: 22132, uat_name: 'Bacău', county_name: 'Bacău', match_confidence: 'safe' });
    expect(t?.matchConfidence).toBe('safe');
    expect(t?.sirutaCode).toBe('22132');
  });
});

describe('M6 — financials.lines money values are strings', () => {
  it('stringifies numeric jsonb values, leaves non-numbers as-is', () => {
    const row = { ...(yearRow(2024, '0.00', '0.00') as unknown as FinancialRow), lines: { Creante: 69341056, Note: 'n/a', Zero: 0 } };
...
describe('M10 — companyResolve honors limit:0', () => {
  it('returns no hits for limit 0 instead of flooring to 1', async () => {
    const resolveByName = vi.fn(async () => ok({ hits: [{ dim: 'name' as const, value: '1', label: 'X', cui: '1', confidence: 1 }], degraded: false }));
...
describe('M14 — resolve hits share one shape across dims', () => {
  it('maps county matches to the structured hit shape (not plain strings)', () => {
    const hits = toCompanyResolveHits({
      dim: 'county', q: 'bac', matches: [], caenMatches: [], countyMatches: ['Bacău'], ambiguous: false, degraded: false,
    });
    expect(hits).toEqual([{ dim: 'COUNTY', value: 'Bacău', label: 'Bacău', cui: null, confidence: null }]);
  });

  it('maps caen matches by code with the same shape', () => {
    const hits = toCompanyResolveHits({
      dim: 'caen', q: '6201', matches: [], caenMatches: [{ code: '6201', rev: 'rev2', label: 'Software' }], countyMatches: [], ambiguous: false, degraded: false,
    });
    expect(hits[0]).toEqual({ dim: 'CAEN', value: '6201', label: 'Software', cui: null, confidence: null });
  });
});
