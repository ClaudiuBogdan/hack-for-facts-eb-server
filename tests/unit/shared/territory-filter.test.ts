...
import { buildTerritoryCuiPredicate, hasTerritoryFilter } from '@/modules/shared/core/filters/territory.js';
...
describe('buildTerritoryCuiPredicate — semijoin shape', () => {
  it('region IN compiles to a parameterized cui semijoin with the territories join', () => {
    ...
    expect(sql).toContain('join core.territories t on t.territorial_siruta_code = pe.territorial_siruta_code');
  ...
  it('siruta matches the canonical territorial_siruta_code join key', () => {
    const { sql, parameters } = compileCondition(buildTerritoryCuiPredicate(CUI_COL, { siruta: ['120726'] })!);
  ...
  it('isUat-only prunes the territories join (the column lives on public_entities)', () => {
    const { sql, parameters } = compileCondition(buildTerritoryCuiPredicate(CUI_COL, { isUat: false })!);
  ...
  it('exclusion negates membership (NOT (… IN (…)))', () => {
    const { sql, parameters } = compileCondition(buildTerritoryCuiPredicate(CUI_COL, { excludeRegion: ['Bucuresti-Ilfov'] })!);
  ...
  it('an empty include array alongside a set dimension compiles to false (match-nothing, never widens)', () => {
    ...
    const { sql } = compileCondition(buildTerritoryCuiPredicate(CUI_COL, { region: [], isUat: true })!);
    expect(sql).toContain('false');
    expect(sql).toContain('pe.is_uat = $1');
  });

  it('no geographic dimension (incl. empty-only arrays) → undefined predicate', () => {
    expect(buildTerritoryCuiPredicate(CUI_COL, {})).toBeUndefined();
    // An empty array carries no membership, so on its own it is "not requested".
    expect(buildTerritoryCuiPredicate(CUI_COL, { region: [] })).toBeUndefined();
  });

  it('rejects an unsafe cui column identifier (spec invariant — throws)', () => {
    expect(() => buildTerritoryCuiPredicate({ alias: 'ces', column: 'cui; drop' }, { region: ['Vest'] })).toThrow();
  });
});
...
describe('hasTerritoryFilter', () => {
  it('is true only when a geographic dimension is set', () => {
    ...
    expect(hasTerritoryFilter({ region: [] })).toBe(false);
