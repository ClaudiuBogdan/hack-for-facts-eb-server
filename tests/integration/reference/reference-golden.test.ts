...
interface PeNode {
  ...
  territory: { territorialSirutaCode: string | null; countyName: string | null; region: string | null; population: number | null } | null;
  fieldTrace: unknown;
}
...
d('Reference golden (live prod)', () => {
  beforeAll(async () => {
    ...
    const built = await buildRedesignApp({ kernelConfig: config.kernel, logLevel: 'silent', modules: ['reference'] });
    app = built.app;
  ...
  it('includeTrace:true populates field_trace', async () => {
    ...
    const e = (res.data as { referencePublicEntity: { fieldTrace: unknown } }).referencePublicEntity;
  ...
  it('list + totalCount + region filter (Nord-Vest = 1,946)', async () => {
    ...
    const c = (res.data as { referencePublicEntities: { edges: unknown[]; totalCount: number } }).referencePublicEntities;
  ...
  it('cursor pagination is keyset-stable (no overlap across pages)', async () => {
    const page1 = await gql(`query{ referencePublicEntities(first:10){ edges{ node{ cui } cursor } pageInfo{ endCursor hasNextPage } } }`);
    const c1 = (page1.data as { referencePublicEntities: { edges: { node: { cui: string } }[]; pageInfo: { endCursor: string | null; hasNextPage: boolean } } }).referencePublicEntities;
    expect(c1.pageInfo.hasNextPage).toBe(true);
    const page2 = await gql(`query($a:String){ referencePublicEntities(first:10, after:$a){ edges{ node{ cui } } } }`, { a: c1.pageInfo.endCursor });
    const c2 = (page2.data as { referencePublicEntities: { edges: { node: { cui: string } }[] } }).referencePublicEntities;
  ...
  it('cursor pagination on a NON-default sort (entity_type) round-trips', async () => {
    ...
    const p1 = await gql(`query{ referencePublicEntities(first:5, sort: entity_type){ edges{ node{ cui } } pageInfo{ endCursor hasNextPage } } }`);
    const c1 = (p1.data as { referencePublicEntities: { edges: { node: { cui: string } }[]; pageInfo: { endCursor: string | null; hasNextPage: boolean } } }).referencePublicEntities;
    expect(c1.pageInfo.hasNextPage).toBe(true);
    const p2 = await gql(`query($a:String){ referencePublicEntities(first:5, sort: entity_type, after:$a){ edges{ node{ cui } } } }`, { a: c1.pageInfo.endCursor });
    expect(p2.errors).toBeUndefined();
    const c2 = (p2.data as { referencePublicEntities: { edges: { node: { cui: string } }[] } }).referencePublicEntities;
    expect(c2.edges.length).toBeGreaterThan(0);
  });

  it('updatedAt serializes as an ISO string (timestamptz ::text cast)', async () => {
    const res = await gql(`query($cui:CUI!){ referencePublicEntity(cui:$cui){ updatedAt } }`, { cui: CLUJ_CUI });
    const u = (res.data as { referencePublicEntity: { updatedAt: string } }).referencePublicEntity.updatedAt;
    expect(res.errors).toBeUndefined();
  ...
  it('sort:updated_at paginates without error (the broken-Date-sort regression)', async () => {
    const p1 = await gql(`query{ referencePublicEntities(first:5, sort: updated_at){ edges{ node{ cui } } totalCount pageInfo{ endCursor hasNextPage } } }`);
    expect(p1.errors).toBeUndefined();
    const c1 = (p1.data as { referencePublicEntities: { totalCount: number; pageInfo: { endCursor: string | null; hasNextPage: boolean } } }).referencePublicEntities;
    // totalCount is the FILTERED total (15,002), NOT "remaining after cursor".
    expect(c1.totalCount).toBe(15002);
    const p2 = await gql(`query($a:String){ referencePublicEntities(first:5, sort: updated_at, after:$a){ edges{ node{ cui } } totalCount } }`, { a: c1.pageInfo.endCursor });
    expect(p2.errors).toBeUndefined();
    // totalCount stays the full denominator on page 2 (review BLOCKER fix).
    expect((p2.data as { referencePublicEntities: { totalCount: number } }).referencePublicEntities.totalCount).toBe(15002);
  });

  it('classification resolve scoped to a system does not leak other systems', async () => {
    const m = await mcpCall('resolve_reference_classification', { system: 'caen_rev2', q: '1', limit: 20 });
  ...
  it('children tree (golden has 85)', async () => {
    const res = await gql(`query($cui:CUI!){ referencePublicEntityChildren(cui:$cui){ cui } }`, { cui: CLUJ_CUI });
    const children = (res.data as { referencePublicEntityChildren: unknown[] }).referencePublicEntityChildren;
  ...
  it('aggregate by entity_type (education = 6,723; 14 buckets)', async () => {
    ...
    const buckets = (res.data as { referencePublicEntityAggregate: { key: string; count: number }[] }).referencePublicEntityAggregate;
  ...
  it('territory detail by siruta + counties (42) + regions (8) rollups', async () => {
    ...
    const d2 = res.data as { referenceTerritory: { name: string } | null; referenceCounties: unknown[]; referenceRegions: unknown[] };
  ...
  it('classification systems + filter (caen_rev2 = 1,675) + code detail round-trip', async () => {
    ...
    const systems = (sys.data as { referenceClassificationSystems: { system: string; count: number }[] }).referenceClassificationSystems;
    expect(systems.find((s) => s.system === 'caen_rev2')?.count).toBe(1675);
    expect(systems.find((s) => s.system === 'caen_rev1')?.count).toBe(785);

    const list = await gql(`query{ referenceClassificationCodes(filter:{ system:{ eq:"caen_rev2" } }, first:1){ edges{ node{ system code } } } }`);
    const node = (list.data as { referenceClassificationCodes: { edges: { node: { system: string; code: string } }[] } }).referenceClassificationCodes.edges[0]?.node;
    expect(node?.system).toBe('caen_rev2');
    // round-trip: fetch the same code by PK
    const one = await gql(`query($s:String!,$c:String!){ referenceClassificationCode(system:$s, code:$c){ system code } }`, { s: node?.system, c: node?.code });
    const got = (one.data as { referenceClassificationCode: { system: string; code: string } | null }).referenceClassificationCode;
    expect(got?.code).toBe(node?.code);
  });

  it('Entity.reference contributor parity (same card via the registry)', async () => {
    const res = await gql(`query($cui:CUI!){ entity(cui:$cui){ cui reference{ cui name isUat entityType } } }`, { cui: CLUJ_CUI });
    const ref = (res.data as { entity: { reference: { cui: string; name: string; isUat: boolean; entityType: string | null } | null } }).entity.reference;
  ...
  it('GraphQL ≡ MCP equivalence for the public-entity card (and MCP omits field_trace)', async () => {
    const g = await gql(`query($cui:CUI!){ referencePublicEntity(cui:$cui){ cui name entityType isUat } }`, { cui: CLUJ_CUI });
    ...
    const item = m.out['item'] as { cui: string; name: string; entityType: string | null; isUat: boolean } | undefined;
  ...
  it('MCP resolve — public_entity name → CUI, territory name → SIRUTA', async () => {
    const pe = await mcpCall('resolve_reference_filter', { dim: 'public_entity', q: 'Cluj-Napoca', limit: 5 });
    ...
    const terr = await mcpCall('resolve_reference_filter', { dim: 'territory', q: 'Cluj-Napoca', limit: 5 });
  ...
  it('field_trace never appears in an MCP list response', async () => {
    const m = await mcpCall('search_reference_public_entities', { filter: { isUat: { eq: true } }, limit: 5 });
    expect(m.raw).not.toContain('field_trace');
