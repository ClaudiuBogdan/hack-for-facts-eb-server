...
const mcpCall = async (name: string, args: Record<string, unknown>): Promise<Record<string, unknown>> => {
  const res = await app.inject({
    method: 'POST',
    url: '/api/v1/mcp',
  ...
  if (body.result?.structuredContent !== undefined) return body.result.structuredContent as Record<string, unknown>;
...
d('budget golden (live prod)', () => {
  beforeAll(async () => {
    ...
    const built = await buildRedesignApp({ kernelConfig: config.kernel, logLevel: 'silent', modules: ['budget'] });
    app = built.app;
  ...
  it('Cluj-Napoca 2025 DETAILED summary matches the pinned MV golden', async () => {
    ...
    const rows = (res.data as { budgetEntitySummary: { year: number; totalIncome: string; totalExpense: string; budgetBalance: string }[] }).budgetEntitySummary;
  ...
  it('tri-surface: GraphQL summary == raw MV SQL == MCP snapshot', async () => {
    const sqlRes = await pool.query<{ total_income: string; total_expense: string; budget_balance: string }>(
    ...
    const g = await gql(`query($cui: CUI!){ budgetEntitySummary(cui:$cui, year:2025, reportType:EXECUTION_DETAILED){ totalIncome totalExpense } }`, { cui: CLUJ });
    const gRow = (g.data as { budgetEntitySummary: { totalIncome: string; totalExpense: string }[] }).budgetEntitySummary[0];
    expect(gRow?.totalIncome).toBe(raw?.total_income);
    expect(gRow?.totalExpense).toBe(raw?.total_expense);

    const mcp = await mcpCall('get_budget_entity_snapshot', { cui: CLUJ, year: 2025, reportType: 'EXECUTION_DETAILED' });
    const item = (mcp['item'] as { execution: { totalIncome: string; totalExpense: string } }).execution;
  ...
  it('classification aggregate (fact path) reconciles with raw SQL over the pruned leaf', async () => {
    ...
    const rows = (g.data as { budgetAggregateByClassification: { functionalCode: string; economicCode: string | null; amount: string }[] }).budgetAggregateByClassification;
  ...
  it('contributor parity: Entity.budget uses the default report type (AGG_PRINCIPAL)', async () => {
    ...
    const b = (g.data as { entity: { budget: { presence: boolean; reportType: string; totalExpense: string } } }).entity.budget;
  ...
  it('commitment MONTHLY summary returns null for gap metrics (no crash — R1 fix)', async () => {
    ...
    const rows = (g.data as { budgetCommitmentSummary: { month: number; crediteAngajament: string | null; receptiiNeplatite: string | null }[] }).budgetCommitmentSummary;
    expect(rows.length).toBeGreaterThan(0);
  ...
  it('aggregate TOTAL_EURO applies the FX factor in SQL (numeric-precise, < RON total)', async () => {
    const ron = await gql(`query($cui: String!){ budgetAggregateByClassification(filter:{ reportingYear:{eq:2025}, reportType:{eq:"EXECUTION_DETAILED"}, accountCategory:{eq:"EXPENSE"}, frequency:{eq:"YEAR"}, entityCuis:{in:[$cui]} }, normalization:TOTAL, limit:1){ amount } }`, { cui: CLUJ });
    const eur = await gql(`query($cui: String!){ budgetAggregateByClassification(filter:{ reportingYear:{eq:2025}, reportType:{eq:"EXECUTION_DETAILED"}, accountCategory:{eq:"EXPENSE"}, frequency:{eq:"YEAR"}, entityCuis:{in:[$cui]} }, normalization:TOTAL_EURO, limit:1){ amount } }`, { cui: CLUJ });
    const ronTop = Number((ron.data as { budgetAggregateByClassification: { amount: string }[] }).budgetAggregateByClassification[0]?.amount);
    const eurTop = Number((eur.data as { budgetAggregateByClassification: { amount: string }[] }).budgetAggregateByClassification[0]?.amount);
  ...
  it('capability gate: vs-execution is empty + a caveat (bgc not loaded)', async () => {
    ...
    const v = (g.data as { budgetVsExecution: { items: unknown[]; caveats: string[] } }).budgetVsExecution;
