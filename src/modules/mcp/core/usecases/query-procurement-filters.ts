...
export async function queryProcurementFilters(
  ...
  if (abstentionReason !== null) {
    return ok({
      ok: true,
      answerClass,
      caveats: buildCaveats(query, quality),
      quality,
      query,
      rows: [],
      status: 'abstained',
  ...
  const rowsResult = await fetchRows(deps, query, quality);
  if (rowsResult.isErr()) {
  ...
  return ok({
    ...
    rows,
    status: 'allowed',
    summary: buildSummary(query, rows.length),
  });
}
...
function buildCaveats(
  ...
  return [
    ...
    ...(!quality.spendRankingsAllowed
      ? [
          `Spend totals are withheld because spend coverage for ${quality.sourceGrain} is below the approved threshold.`,
        ]
      : []),
...
async function fetchRows(
  ...
  query: NormalizedProcurementInput,
  quality: ProcurementAggregateQuality
): Promise<Result<QueryProcurementFiltersOutput['rows'], McpError>> {
  if (query.analysis === 'top_suppliers') {
    const rows = await deps.procurementRepo.rankSuppliers(query);
    return rows.map((value) => redactSupplierSpendFields(value, quality));
  }

  if (query.analysis === 'category_breakdown') {
    const rows = await deps.procurementRepo.rankCpvDivisions(query);
    return rows.map((value) => redactCategorySpendFields(value, quality));
  }

  const rows = await deps.procurementRepo.listSameDayDirectAcquisitionCandidates(query);
  return rows.map((value) => redactSameDaySpendFields(value, quality));
}

function redactSupplierSpendFields(
  rows: ProcurementSupplierRankingRow[],
  quality: ProcurementAggregateQuality
): ProcurementSupplierRankingRow[] {
  if (quality.spendRankingsAllowed) {
    return rows;
  }

  return rows.map((row) => ({ ...row, amountRonSum: null }));
}

function redactCategorySpendFields(
  rows: ProcurementCategoryBreakdownRow[],
  quality: ProcurementAggregateQuality
): ProcurementCategoryBreakdownRow[] {
  if (quality.spendRankingsAllowed) {
    return rows;
...
function redactSameDaySpendFields(
  rows: ProcurementSameDayCandidateRow[],
  quality: ProcurementAggregateQuality
): ProcurementSameDayCandidateRow[] {
  if (quality.spendRankingsAllowed) {
    return rows;
