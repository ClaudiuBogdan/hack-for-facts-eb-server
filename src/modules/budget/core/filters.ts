...
import { ACCOUNT_CATEGORIES, BUDGET_FREQUENCIES, COMMITMENT_REPORT_TYPES, EXECUTION_REPORT_TYPES } from './constants.js';
...
export const budgetFactFilterSpec: CollectionFilterSpec = {
  ...
  fields: [
    ...
    { name: 'entityCuis', type: 'string', ops: ['in', 'eq'], array: true, column: { alias: 'eli', column: 'entity_cui' } },
    { name: 'mainCreditorCui', type: 'string', ops: ['eq'], column: { alias: 'eli', column: 'main_creditor_cui' } },
    // ── classification / dimensions (identity index + text_pattern_ops)
    { name: 'budgetSectorIds', type: 'int', ops: ['in', 'eq'], array: true, column: { alias: 'eli', column: 'budget_sector_id' } },
    { name: 'fundingSourceIds', type: 'int', ops: ['in', 'eq'], array: true, column: { alias: 'eli', column: 'funding_source_id' } },
    { name: 'expenseTypes', type: 'string', ops: ['in', 'eq'], array: true, exclude: true, column: { alias: 'eli', column: 'expense_type' } },
    { name: 'functionalCodes', type: 'string', ops: ['in', 'eq'], array: true, exclude: true, column: { alias: 'eli', column: 'functional_code' } },
    { name: 'functionalPrefix', type: 'string', ops: ['prefix'], exclude: true, column: { alias: 'eli', column: 'functional_code' }, description: 'LIKE prefix% on the text_pattern_ops btree.' },
    { name: 'economicCodes', type: 'string', ops: ['in', 'eq'], array: true, exclude: true, column: { alias: 'eli', column: 'economic_code' } },
    { name: 'economicPrefix', type: 'string', ops: ['prefix'], exclude: true, column: { alias: 'eli', column: 'economic_code' }, description: 'LIKE prefix% on the text_pattern_ops btree.' },
    { name: 'programCodes', type: 'string', ops: ['in', 'eq'], array: true, exclude: true, column: { alias: 'eli', column: 'program_code' } },
    // ── amount (row-level; money type → exact ::numeric compare on the freq column)
    { name: 'minAmount', type: 'money', ops: ['gte'], column: { alias: 'eli', column: 'amount_virtual' }, description: 'Lower bound on the frequency amount column (NULL-excluding).' },
    { name: 'maxAmount', type: 'money', ops: ['lte'], column: { alias: 'eli', column: 'amount_virtual' }, description: 'Upper bound on the frequency amount column.' },
    // ── transfer exclusion (virtual; opt-in; fact path only — MVs pre-exclude)
    { name: 'excludeTransfers', type: 'bool', ops: ['eq'], column: { alias: 'eli', column: 'exclude_transfers_virtual' }, description: 'Exclude transfer codes (the exact set the MVs bake in).' },
    // ── core-join filters (entity / territory; need the LEFT JOINs)
    { name: 'entityTypes', type: 'string', ops: ['in', 'eq'], array: true, column: { alias: 'e', column: 'entity_type' }, description: 'core.public_entities.entity_type (needs entity join).' },
    { name: 'isUat', type: 'bool', ops: ['eq'], column: { alias: 'e', column: 'is_uat' } },
    { name: 'countyCodes', type: 'string', ops: ['in', 'eq'], array: true, exclude: true, column: { alias: 't', column: 'county_code' }, description: 'Resolves via the territory join.' },
    { name: 'regions', type: 'string', ops: ['in', 'eq'], array: true, exclude: true, column: { alias: 't', column: 'region' } },
    { name: 'minPopulation', type: 'int', ops: ['gte'], column: { alias: 't', column: 'population' } },
    { name: 'maxPopulation', type: 'int', ops: ['lte'], column: { alias: 't', column: 'population' } },
    { name: 'q', type: 'string', ops: ['contains'], column: { alias: 'e', column: 'name' }, description: 'Entity-name trigram (Postgres pg_trgm; not Meili/OS on this collection).' },
    // ── exclusion-only fields
    { name: 'reportIds', type: 'string', ops: ['in'], array: true, exclude: true, column: { alias: 'eli', column: 'report_id' }, description: 'Exclusion only.' },
    { name: 'excludeEntityCuis', type: 'string', ops: ['in'], array: true, exclude: true, column: { alias: 'eli', column: 'entity_cui' }, description: 'Exclusion only.' },
  ],
...
export const budgetCommitmentFactFilterSpec: CollectionFilterSpec = {
  ...
  fields: [
    ...
    },
    {
      ...
      description: 'LIST L2 partition prune (mandatory; commitment partition literal; repo-intercepted).',
    },
    {
      name: 'frequency',
      type: 'enum',
    ...
    { name: 'months', type: 'int', ops: ['in'], array: true, column: { alias: 'cli', column: 'reporting_month' } },
    { name: 'quarters', type: 'int', ops: ['in'], array: true, column: { alias: 'cli', column: 'quarter' } },
    { name: 'entityCuis', type: 'string', ops: ['in', 'eq'], array: true, column: { alias: 'cli', column: 'entity_cui' } },
    { name: 'mainCreditorCui', type: 'string', ops: ['eq'], column: { alias: 'cli', column: 'main_creditor_cui' } },
    { name: 'budgetSectorIds', type: 'int', ops: ['in', 'eq'], array: true, column: { alias: 'cli', column: 'budget_sector_id' } },
    { name: 'fundingSourceIds', type: 'int', ops: ['in', 'eq'], array: true, column: { alias: 'cli', column: 'funding_source_id' } },
    { name: 'functionalCodes', type: 'string', ops: ['in', 'eq'], array: true, exclude: true, column: { alias: 'cli', column: 'functional_code' } },
    { name: 'functionalPrefix', type: 'string', ops: ['prefix'], exclude: true, column: { alias: 'cli', column: 'functional_code' } },
    { name: 'economicCodes', type: 'string', ops: ['in', 'eq'], array: true, exclude: true, column: { alias: 'cli', column: 'economic_code' } },
    { name: 'economicPrefix', type: 'string', ops: ['prefix'], exclude: true, column: { alias: 'cli', column: 'economic_code' } },
    // ── core-join filters (commitment rows carry entity_cui; R2 review fix) ──
    { name: 'entityTypes', type: 'string', ops: ['in', 'eq'], array: true, column: { alias: 'e', column: 'entity_type' } },
    { name: 'isUat', type: 'bool', ops: ['eq'], column: { alias: 'e', column: 'is_uat' } },
    { name: 'countyCodes', type: 'string', ops: ['in', 'eq'], array: true, exclude: true, column: { alias: 't', column: 'county_code' } },
    { name: 'regions', type: 'string', ops: ['in', 'eq'], array: true, exclude: true, column: { alias: 't', column: 'region' } },
    { name: 'q', type: 'string', ops: ['contains'], column: { alias: 'e', column: 'name' }, description: 'Entity-name trigram.' },
    { name: 'excludeEntityCuis', type: 'string', ops: ['in'], array: true, exclude: true, column: { alias: 'cli', column: 'entity_cui' }, description: 'Exclusion only.' },
  ],
  sort: { default: 'LINE_ORDER', allowed: ['LINE_ORDER', 'AMOUNT_DESC', 'AMOUNT_ASC'] },
};
...
export const budgetRankingFilterSpec: CollectionFilterSpec = {
  ...
  fields: [
    ...
    { name: 'month', type: 'int', ops: ['eq', 'in'], array: true, column: { alias: 'mv', column: 'month' } },
    { name: 'quarter', type: 'int', ops: ['eq', 'in'], array: true, column: { alias: 'mv', column: 'quarter' } },
    { name: 'entityCuis', type: 'string', ops: ['in', 'eq'], array: true, column: { alias: 'mv', column: 'entity_cui' } },
    { name: 'mainCreditorCui', type: 'string', ops: ['eq'], column: { alias: 'mv', column: 'main_creditor_cui' } },
    // ── geo / entity filters resolve via the core join (entity + territory)
    { name: 'countyCodes', type: 'string', ops: ['in', 'eq'], array: true, column: { alias: 't', column: 'county_code' } },
    { name: 'regions', type: 'string', ops: ['in', 'eq'], array: true, column: { alias: 't', column: 'region' } },
    { name: 'isUat', type: 'bool', ops: ['eq'], column: { alias: 'e', column: 'is_uat' } },
    { name: 'minPopulation', type: 'int', ops: ['gte'], column: { alias: 't', column: 'population' } },
    { name: 'maxPopulation', type: 'int', ops: ['lte'], column: { alias: 't', column: 'population' } },
  ],
  sort: { default: 'EXPENSE', allowed: ['INCOME', 'EXPENSE', 'BALANCE', 'PER_CAPITA', 'ENTITY_NAME', 'POPULATION', 'COUNTY'] },
};
...
export const budgetReportFilterSpec: CollectionFilterSpec = {
  ...
  fields: [
    { name: 'entityCui', type: 'string', ops: ['eq', 'in'], array: true, column: { alias: 'r', column: 'entity_cui' } },
    { name: 'reportingYear', type: 'int', ops: ['eq', 'in', 'between'], column: { alias: 'r', column: 'reporting_year' } },
    { name: 'reportType', type: 'string', ops: ['eq', 'in'], array: true, column: { alias: 'r', column: 'report_type' } },
    { name: 'mainCreditorCui', type: 'string', ops: ['eq'], column: { alias: 'r', column: 'main_creditor_cui' } },
    { name: 'reportingPeriod', type: 'string', ops: ['eq', 'in'], array: true, column: { alias: 'r', column: 'reporting_period' } },
    { name: 'reportDate', type: 'date', ops: ['between'], column: { alias: 'r', column: 'report_date' } },
  ],
...
export const budgetApprovedFactFilterSpec: CollectionFilterSpec = {
  ...
  fields: [
    { name: 'budgetYear', type: 'int', ops: ['eq', 'in', 'between'], column: { alias: 'af', column: 'budget_year' } },
    { name: 'measureYear', type: 'int', ops: ['eq'], column: { alias: 'af', column: 'measure_year' } },
    { name: 'measureKind', type: 'string', ops: ['eq', 'in'], array: true, column: { alias: 'af', column: 'measure_kind' } },
    { name: 'budgetComponent', type: 'string', ops: ['eq', 'in'], array: true, column: { alias: 'af', column: 'budget_component' } },
    { name: 'functionalCode', type: 'string', ops: ['eq', 'prefix'], column: { alias: 'af', column: 'functional_code' } },
    { name: 'economicCode', type: 'string', ops: ['eq', 'prefix'], column: { alias: 'af', column: 'economic_code' } },
  ],
