...
const objectsAndQuery = /* GraphQL */ `
  ...
  type ParliamentVote {
    ...
    divisionNumber: Int
    "DEPRECATED as the vote-to-bill answer; read resolutionStatus + bill instead. The legacy single-valued column, kept so existing consumers do not break. It disagrees with the resolution contract in BOTH directions: 860 votes the resolver RESOLVED carry a null billKey here, and it carries no statement at all about whether the resolver actually asserted a bill."
    billKey: ID
    "The bill this division was about, and ONLY when the resolver actually asserted one - resolutionStatus 'resolved' or 'adjudicated', taken from resolvedDisplayBillKey. Null for 'unresolved' (the resolver abstained: no evidence), null for 'conflict' (evidence points at 2-3 DIFFERENT dossiers - see voteLinks for the competing observations), and null while a row is unstamped. Do NOT fall back to billKey when this is null: that is precisely the false assertion this field exists to stop."
    bill: ParliamentBill
    "Why the vote-to-bill answer is what it is. 'resolved' (12,512 votes) - asserted, read bill. 'unresolved' (8,341) - the resolver ABSTAINED for want of evidence; this is NOT 'the division had no bill', it is 'we will not claim one', and a reader cannot tell those apart without this field. 'conflict' (18) - the evidence points at two or three different dossiers; voteLinks still shows them as observations but none of them is THE bill. 'adjudicated' - a human decided. Null - the resolver has not stamped this row yet (the derive is all-or-nothing, so a blocked batch leaves whole cohorts unstamped). Measured 2026-08-08."
    resolutionStatus: String
    "How the resolution was reached, for readers who need to weigh it: 'agree' (both event and attrs evidence, 9,448), 'attrs_only' (2,168) and 'event_only' (860) are single-source and uncorroborated, 'agree_disambiguated' (38), 'resolved_by_event_tally' (3), 'no_evidence' (8,341, paired with unresolved), 'conflict_event_ambiguous_no_attrs' (18). Null while unstamped."
    resolutionMethod: String
    "Bill-link OBSERVATIONS for this division, WITH the role of each edge - the only place a vote states what it was procedurally FOR. These are EVIDENCE, not the answer: for the authoritative vote-to-bill statement read resolutionStatus and bill. A vote whose resolutionStatus is 'conflict' still lists 2-3 links here spanning DIFFERENT dossiers (18 votes measured 2026-08-08), and presenting any one of them as the bill would assert something the resolver explicitly refused to. Prefer it to billKey: that column holds at most one bill and carries no role, while 1,502 divisions link to two. Empty for the 8,408 divisions with no bill link at all. IMPORTANT: role names the MOTION ON THE FLOOR, not the result. A final_adoption motion can be voted DOWN (441 links) and a final_rejection motion can FAIL (8). The bill's fate is role composed with tallyRelation, never either alone."
  ...
  type ParliamentBill {
    ...
    "WHICH chamber casts the final, unappealable vote on this bill (art. 75 of the Constitution) — the single fact that says where the bill's fate is decided. 16,421 of 41,990 bills; null where the source printed none (incl. the 21 rows printing the '-' placeholder). OPEN STRING, NOT an enum: 16,410 rows carry 'Camera Deputaţilor' (13,196), 'Senatul' (2,874) or 'Camera Deputaţilor + Senatul' (340), the other 11 carry the long art.75 sentence instead. On 7 of those 11 the SOURCE ITSELF is corrupt: cdep.ro emits an MP's name run together with the article reference as literal text ('…prevazuta la Emil Bocart.55 alin.2'), with no element boundary to split on — verified in the stored captures for all 11, so this is NOT a parser defect and cannot be repaired by parsing. The remaining 4 are clean, merely long. Match the known vocabulary before rendering; show unrecognised values as plain text or not at all."
