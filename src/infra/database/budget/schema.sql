...
-- Table to store curated scraped website profile data for entities
CREATE TABLE EntityProfiles (
    cui VARCHAR(20) PRIMARY KEY,
...
CREATE INDEX idx_entity_profiles_county_code ON EntityProfiles (county_code)
    WHERE county_code IS NOT NULL;
CREATE INDEX idx_entity_profiles_institution_type ON EntityProfiles (institution_type)
    WHERE institution_type IS NOT NULL;
CREATE INDEX idx_entity_profiles_county_code_type ON EntityProfiles (county_code, institution_type)
    WHERE county_code IS NOT NULL;
CREATE INDEX idx_entity_profiles_scraped_at ON EntityProfiles (scraped_at);
CREATE INDEX idx_entity_profiles_full_profile_gin ON EntityProfiles USING gin (full_profile jsonb_path_ops);
...
CREATE INDEX idx_gin_entity_profiles_name ON EntityProfiles USING gin (institution_name gin_trgm_ops);
