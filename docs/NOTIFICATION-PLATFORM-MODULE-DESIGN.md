...
// permanent_failed is terminal for normal worker/provider flow and remains in
// dead-letter search. Audited admin requeue is the sole permanent_failed → ready
// transition; unlike unknown → ready, it does not require duplicate-risk acknowledgement.
...
export interface DeliveryRepo {
  ...
  cancelPendingForUser(input: {
    ...
    kindIds?: readonly string[]; // provided ⇒ kind_id = ANY(kindIds); absent ⇒ all kinds
    reason: string;
    now: Date;
...
export interface SendJobScheduler {
  enqueue(
    p: SendJobPayload,
    opts?: { delayMs?: number; dedupeToken?: string }
...
| `resolveEventRecipients`                                                             | `{events, registry, subscriptions, preferences, anonymization, logicalNotifications, deliveries, digests, renderScheduler, audit}` → `{eventId, pageSize?}` → `{created, skipped, resumed}`     | Claim event (lease); page subscribers 500/keyset from persisted cursor; skip anonymized users before eligibility; `evaluateEligibility` per user; `projectContent` for the inbox snapshot; idempotent logical inserts; `planChannelDeliveries` for every eligible persisted logical in the page, including replay duplicates, to repair missing idempotent delivery/digest work; persist cursor per page; mark resolved. Only newly created logicals emit `recipient.included`/`logical.created`; skips are audited and redacted.                             |
...
| `renderDelivery`                                                                     | `{deliveries, digests, logicalNotifications, registry, events, channelAdapters}` → `{deliveryId}` → `{rendered}`                                                                                | Claim `pending_render`; an individual delivery re-projects persisted event facts and calls `adapter.render`. A digest loads its rendered batch, calls `listMembersNewestFirst` with the 20-item cap, fetches exactly `renderedItemIds` through `findByIds`, restores that authoritative snapshot order, and calls `adapter.renderDigest`; fresh membership ordering never replaces the snapshot. Save content + hash as usual; → `scheduled` (not_before in future) else `ready` + enqueue send (active mode only).                                           |
| `dispatchDelivery`                                                                   | `{deliveries, attempts, destinations, preferences, anonymization, registry, channelAdapters, sendScheduler, audit}` → `{deliveryId}` → `DispatchOutcome`                                        | Stream-gated claim increments `attempt_count`; cancel/audit `user_anonymized` before provider contact; pre-send recheck order (spec §6.3) is preferences → suppression → expiry → re-resolved destination fingerprint (mismatch ⇒ `destination_changed`); create attempt row using the claimed row's incremented `attemptCount` BEFORE provider call; accepted → `accepted`; transient → `retry_wait` + delayed re-enqueue; permanent → terminal `permanent_failed`, visible in dead-letter search; ambiguous → claim-token-fenced `resolveAmbiguousOutcome`. |
| `applyProviderOutcome`                                                               | `{deliveries, destinations, audit}` → `{providerRef?/deliveryId?/idempotencyKey?, outcome: 'delivered'\|'bounced'\|'complained'\|'delayed', occurredAt, destinationFingerprint?}` → `{applied}` | Match by `providerRef`, then `deliveryId`, then the legacy idempotency-key fallback; a delivery-tag match persists the provider ref. Monotonic transition via state machine; hard bounce/complaint ⇒ suppress by (channel, fingerprint) — never rewrites preference; delayed, duplicate, and out-of-order outcomes are state no-ops.                                                                                                                                                                                                                          |
| `resolveAmbiguousOutcome`                                                            | `{deliveries, attempts, channelAdapters, audit}` → `{deliveryId, expectedClaimToken}` → `{resolution: 'accepted'\|'retried'\|'unknown'}`                                                        | Reuse idempotency key inside window → adapter.reconcile → persisted webhooks → else `unknown` + dead letter (no auto-resend). Every transition carries the caller's claim token; losing the fence is a successful no-op.                                                                                                                                                                                                                                                                                                                                      |
| `cancelPendingExternalForUser`                                                       | `{deliveries, digests, registry, audit}` → `{userId, channels?, reason}` → `{cancelled}`                                                                                                        | Computes optional kind IDs as registry entries whose `preferenceClass !== 'required'` and cancels only matching non-terminal external work; inbox items untouched. Used by preference-off and global pause.                                                                                                                                                                                                                                                                                                                                                   |
...
| `requeueDeadLetter`                                                                  | `{deliveries, sendScheduler, audit}` → `{deliveryId, adminUserId, reason, acknowledgeDuplicateRisk}` → `{requeued}`                                                                             | Audited; accepts `dead_letter`, `permanent_failed`, and `unknown`; only `unknown` requires the explicit duplicate-risk acknowledgement.                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `getShadowComparison`                                                                | `{deliveries, logicalNotifications, legacyOutboxReader}` → `{kindId, periodKey?}` → `ShadowComparisonSummary`                                                                                   | Recipient set + content-hash parity vs the legacy outbox. Phase 4's reader must emit comparable normalized framed hashes; direct equality with legacy raw stored hashes is not meaningful.                                                                                                                                                                                                                                                                                                                                                                    |
...
| `applyRetention`                                                                     | `{retention repos}` → `{batchLimit}` → `RetentionSummary`                                                                                                                                       | Delete 90d attempts/webhooks; dependency-ordered delete of expired digest members, deliveries, digest batches, logicals, and then unreferenced events at 2y. Scrub expired events only while still referenced. Redacted audit is kept indefinitely; batches are restartable.                                                                                                                                                                                                                                                                                  |
...
The ordering guarantee (spec §11.3) is enforced here, not by queue topology. A delivery in an ordered stream is claimable only when every lower-sequence delivery in the same `(user, channel, stream)` is accepted or terminal:

```sql
UPDATE notification_deliveries d
SET status = 'sending', attempt_count = attempt_count + 1,
    claim_token = $1, claim_expires_at = $2, updated_at = now()
WHERE d.id = $3
  ...
  AND (d.stream_key IS NULL OR NOT EXISTS (
        ...
        WHERE p.user_id = d.user_id AND p.channel = d.channel
          ...
          AND p.status NOT IN ('accepted','delivered','bounced','complained','suppressed','cancelled',
                               'expired','permanent_failed','dead_letter','unknown')))
RETURNING *;
```

A not-yet-eligible delivery simply fails to claim (no-op job); the recovery scan re-dispatches it. Provider acceptance releases the stream because it fixes dispatch order; waiting for a delivery webhook would stall the successor without improving the ordering guarantee.
...
| `NP_SEND`      | `np-delivery-send`      | `{deliveryId}`       | on demand + delayed; job id `np-send-{deliveryId}` initially and `np-send-{deliveryId}-{dedupeToken}` for retry/recovery attempts |
...
export interface NotificationPlatformRuntimeConfig {
  ...
  factories?: Partial<NotificationPlatformRuntimeFactories>; // typed lifecycle/worker test seam
}
...
Eleven Kysely repos, one per port: `KyselyNotificationEventRepo`, `KyselySourceWatermarkRepo`, `KyselySubscriptionRepo`, `KyselyPreferenceRepo`, `KyselyLogicalNotificationRepo`, `KyselyDeliveryRepo` (owns the stream-gated claim), `KyselyDeliveryAttemptRepo`, `KyselyChannelDestinationRepo`, `KyselyDigestBatchRepo`, `KyselyAuditLedgerRepo`, and the SHA-256-audit-backed `AnonymizationCheckPort` adapter. Each exports `makeXRepo(db: UserDbClient): XPort`. Claims use conditional `UPDATE … WHERE status IN (…) RETURNING *` compare-and-set with `claim_token` fencing, following the verified pattern in `src/modules/notification-delivery/shell/repo/delivery-repo.ts`.
...
`resolveDestination` fetches the address from Clerk, normalizes it, computes the HMAC fingerprint, and returns the plaintext only inside `ResolvedDestination` (in-memory). Error classification trusts only the email client's explicit ambiguity flag: timeout/429/5xx and retryable unknowns → transient; 4xx validation and non-retryable unknowns → permanent. Content hashes use UTF-8 byte-length-prefixed subject/HTML/text segments before SHA-256 so tuple boundaries are unambiguous.

### 5.3 Webhook side effect (`shell/webhook/`)

`makeResendPlatformWebhookSideEffect(deps)` is registered **alongside** the legacy side effect in `src/modules/resend-webhooks/shell/combine-side-effects.ts` (persist-first ingestion is already done by that module). It matches `provider_ref` first, then the outbound `delivery_id` tag, persisting the webhook email id as `provider_ref` on a tag match. Only permanent/hard bounces and complaints suppress the destination; soft/temporary bounces and delivery-delayed events do not change delivery state or suppression.
...
| Durability & recovery       | Unit: `runIngestionScan` watermark correctness across restart/overlap (test clock + fault plan); `recoverPlatformWork` finder→re-enqueue behavior and real multi-stage happy/retry/ordered-stream/digest flows via the in-memory job runtime. E2E: recovery finder SQL against real rows.                                                                                                                                   |
...
| Privacy & administration    | Unit: redaction defaults in trace/search results; reveal requires reason + emits audit. Integration: admin route authorization. E2E: anonymizer coverage plus retention deletion of expired user-linked rows, failed/conflicted/unreferenced events, preservation of audit rows, and untouched fresh rows.                                                                                                                  |
