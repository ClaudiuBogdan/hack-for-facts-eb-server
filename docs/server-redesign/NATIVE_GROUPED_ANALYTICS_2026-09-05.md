...
Population remains transitional _(superseded 2026-09-09: the X/F6 consolidation shipped the exact-year adapters — `budget/shell/native` reads the kernel `AnnualPopulationPort` served by `ins-native`'s admitted POP107D publication; see `docs/reviews/chronos-migration-2026-09-09/xf6-population-consolidation-design.md`)_: entity SQL consumes a territory/year relation whose only shipped adapter broadcasts the S1b snapshot. An injected two-year fixture proves division-before-sum and missing-year rejection. Classification currently loads one checked scope scalar and broadcasts it. Neither proves annual custody, matching national/scoped concepts, immutable projected vintage, or a release-consistent snapshot across separate reads. Those remain mandatory before final migration acceptance; no annual adapter is selectable yet.
