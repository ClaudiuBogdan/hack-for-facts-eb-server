...
| :-- | :----------------------------------------------------------------- | :------------- | :------------------------------------------------------------------------------- | :-------------------------- |
| 1   | create at expected revision zero                                   | module-release | `planners/plan-create-or-replace.test` — creates at rev 0                        | unit                        |
| 2   | concurrent creation of the same logical key                        | module-release | `mutation-port.contract` — create race, one winner                               | contract-real               |
| 3   | concurrent replacement, exactly one expected revision succeeds     | module-release | `mutation-port.contract` — CAS race with phase barriers                          | contract-real               |
| 4   | accepted identical payload as a new command                        | module-release | `mutation-port.contract` — identical payload, new revision                       | contract                    |
| 5   | exact retry of the same idempotency key                            | module-release | `mutation-port.contract` — replay returns original result                        | contract                    |
| 6   | idempotency key reuse with different content                       | module-release | `mutation-port.contract` — idempotencyConflict                                   | contract                    |
| 7   | cross-owner reuse of one requester key (privileged)                | module-release | `mutation-port.contract` — receipt race under different owner locks              | contract-real               |
| 8   | receipt expiry and stale CAS protection                            | module-release | `mutation-port.contract` — expired receipt ⇒ revisionConflict, no replay         | contract                    |
| 9   | schema compatibility window and upgrade-required response          | module-release | `replace-record.test` — writeDisabled version; `owner-routes` 409                | unit + integration          |
| 10  | explicit schema migration and failed migration rollback            | module-release | `migrate-record.test` + `mutation-port.contract` — failed commit rolls back      | unit + contract             |
| 11  | delete, tombstone sync, restore, forbidden key reuse               | module-release | `mutation-port.contract` + `sync-records.test`                                   | contract + unit             |
| 12  | current-state pagination while records change between pages        | module-release | `read-port.contract` — cycle stability under interleaved writes                  | contract-real               |
| 13  | exact reconstruction at each verified revision                     | module-release | `read-port.contract` — after-image at revision N                                 | contract                    |
| 14  | latest event and current-state parity                              | module-release | `reconcile-store.test` + `mutation-port.contract` parity case                    | unit + contract             |
| 15  | owner isolation and category-specific admin access                 | module-release | `owner-routes.integration` — cross-owner 404; `admin-routes.integration`         | integration                 |
| 16  | cross-campaign admin isolation; no-permission category has no path | module-release | `admin-routes.integration` — funky admin denied on learning.progress             | integration                 |
| 17  | ordinary application attempts to update or delete events           | module-release | `append-only.e2e` — forged flag, marker-only, allowed redaction, mixed columns   | contract-real               |
| 18  | payload byte, depth, string, and collection limits                 | module-release | `planners/shared.test` — limit table                                             | unit                        |
| 19  | category PII redaction including annotation namespace redactors    | module-release | `anonymize-user-data.test` + `erasure-port.contract`                             | unit + contract-real        |
| 20  | annotation write by allowed actor; forbidden actor rejected        | module-release | `plan-annotate.test` + `annotate-record.test`                                    | unit                        |
| 21  | owner replacement preserves existing annotations untouched         | module-release | `plan-create-or-replace.test` — annotations copied verbatim                      | unit                        |
| 22  | annotation writes increment revision and appear in sync            | module-release | `mutation-port.contract` + `sync-records.test`                                   | contract                    |
| 23  | admin annotation events carry identity and reason                  | module-release | `plan-annotate.test` — admin attribution; DDL CHECK                              | unit + contract             |
| 24  | per-owner record quota and rate-limit enforcement, typed errors    | module-release | `mutation-port.contract` — quota under lock; `replace-record.test` — RateLimited | contract-real + unit        |
| 25  | concurrent same-owner writes never skip a sync row                 | module-release | `sync-race.e2e` — in-flight seq 100 / committed 101 with barriers                | contract-real               |
| 26  | replay of an accepted mutation is not rate-limited                 | module-release | `replace-record.test` — probeReceipt short-circuits limiter                      | unit                        |
| 27  | idempotent Clerk deletion across records, events, receipts         | module-release | `erasure-port.contract` — double-run no-op; `clerk-webhooks.integration`         | contract-real + integration |
