...
**Date:** 2026-07-12
**Status:** Wave 1 implemented (production build 2 remains matrix v1). The API
remediation is committed locally; matrix v2 is not published and the server
remains undeployed. See
...
- **Active production matrix** remains v1 on build 2 (`1ce871d54e28…f412`). The remediation introduces `procurement-analysis-combinations-v2.json`: an exhaustive 554-row, measure-specific closure pinned at `9f552ef40b54…bdf9`. Publishing a new generation is a separate permission-gated rollout step.
...
### Server (`hack-for-facts-eb-server` — committed locally, NOT deployed)
...
| flows→MV aggregate stack + capability gate                                                                            | `analysis_facts_*` + generation-stamped rollups                                              | deprecated aggregate server path removed before deployment; base record/search paths remain    |
| 9 redesign MCP tools plus legacy `/mcp` procurement aggregate                                                         | four retained redesign tools: resolve, two record searches, and `aggregate_procurement`      | six deprecated redesign names and legacy `query_procurement_filters` removed before deployment |
...
6. **Money abstains honestly until M1** — measured awarded-value coverage is 0.762 for contracts and 0.678 for DAs, below the strict 0.95 spend gate. Duplicate-group rescue recovered zero contract values and only 80 DA values, so the hypothesized contract lift to ~0.87 did not occur. Count answers serve; time answers serve degraded-with-disclosure where ≥0.50. The fix is M1 data repair, never threshold loosening.

Implementation-time additions: shape-specific rollup routing preferences (stats/series smallest-first; breakdown/concentration entity-first) mirrored exactly between the artifact generator and the server router; fully-pinned concentration scopes rejected as stats answers; breakdown over a scope-fixed dimension rejected (single bucket). The remediation adds strict transport inputs, semantic month and `topN` validation, matrix-v2 distinct exclusions, boot/runtime hash enforcement, typed answerability/reasons, timeout mapping, and legacy aggregate deletion.
...
- **Server rounds** — initial findings (including two P0s) are addressed: per-request generation pinning; authoritative rollback-aware single-flight cache; exhaustive artifact acceptance and explicit compound supplier-fixed concentration rejection controls; gate composition for time/geo on all shapes with nullable (never fabricated) envelope reads; strict-subset + normalized-window share validation; facets explicit-grain requirement; concentration disclosure semantics; null-money preservation; undated-only keys excluded from rankings; boundary/cache and real-transport tests. The final dual-review gate is rerun after every material change. **Recorded rebuttal:** no server-side disposable-Postgres DDL suite (would duplicate scraper-owned migrations); live SQL is owned by the strict prod golden plus scraper pg suites.
- **Strict live harness (pending matrix-v2 publication)** — the checked-in golden corpus derives from the pinned matrix rather than a hand-picked list: 388 advertised rollup rows expand to 846 GraphQL/MCP/raw-SQL checks after series buckets and concentration bases. Each accepted scope tuple is selected from populated rows in its advertised rollup, so an empty hard-coded fixture cannot create false reconciliation. The checked-in performance gate uses three cache-warmup-disabled fresh server processes per case, records and verifies every response build ID, enforces per-child deadlines, reports per-case cold/warm percentiles, runs a contention probe, and executes the repository-equivalent `EXPLAIN (ANALYZE, BUFFERS)` for the measured worst accepted distinct series. Neither live gate can pass until a matrix-v2 generation is published.
...
- Build 2 publish evidence is recorded above; the next committed run is the separately permission-gated matrix-v2 publication.
- Matrix-v2 query-validation and strict live golden/performance evidence: pending that publication; absence blocks server deployment approval and rollout.
- Build 2 shadow comparison versus the old MV matched top suppliers to the cent on the same-population slice; matrix-v2 rerun remains part of the live gate.
- CronJob smoke remains pending and the CronJob remains suspended by operator policy.

## 6. Unresolved limitations (wave-2+ candidates)

1. **Money abstention until M1 repairs** (headline; see §3.6). The remediated server has no legacy procurement aggregate path: money answers abstain from generation quality rather than consulting the stale old-MV gate. F1 remains a data/M1 concern only for separately operated old projections.
2. **Supplier-scoped concentration is structurally rejected** because the measured supplier key is fixed; simple and compound supplier-fixed scopes have explicit matrix-v2 rejection rows.
...
- Server: local artifact-byte drift is boot-fatal. Analysis requests fail closed without an active generation or when its `matrix_hash` differs from `ANALYSIS_MATRIX_SHA256`; retained record APIs remain available.
